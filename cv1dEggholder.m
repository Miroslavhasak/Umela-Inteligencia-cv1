% Upravte GA tak, aby mal malý selektívny tlak a malú diverzitu.
...
for r = 1:runs
    ...
    for gen = 1:numgen
        ...
        Best = selbest(Pop, Fit, [1,0]);
        Old = selrand(Pop, Fit, 25);

        Work1 = selsus(Pop, Fit, 12);
        Work2 = selsus(Pop, Fit, 12);

        Work1 = crossov(Work1, 1, 0);

        Work2 = mutx(Work2, 0.05, Space);
        Work2 = muta(Work2, 0.05, Delta, Space);
